...
class _WebViewLessonScreenState extends ConsumerState<WebViewLessonScreen> {
  ...
  void _onVideoEvent(JavaScriptMessage msg) {
    ...
    switch (data) {
      ...
      default:
        ...
        } else if (data.startsWith('pdf_inline:')) {
          _openPdf(data.substring('pdf_inline:'.length));
        } else if (data.startsWith('pdf:')) {
          _openPdf(data.substring(4));
        } else if (data.startsWith('open:')) {
          _openExternal(data.substring(5));
        }
    }
  }
  ...
  Future<void> _openPdf(String url) async {
    ...
    if (!mounted || cleanUrl.isEmpty) return;
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => PdfViewerScreen(url: cleanUrl, title: _current.title),
      ),
    );
  }
